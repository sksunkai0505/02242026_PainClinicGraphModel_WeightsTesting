...
def greedy1_MWIS(graph, pi, threshold, n_shuffle, all_solutions=False):
    EPS = 1e-9
    assert n_shuffle > 0
    ...
    best_score_set, best_score = tuple(), 0.0

    while n_shuffle:
        mwis_set, mwis_weight = tuple(), 0.0

        for elem in pi_list:
            feasible_iter = (not graph.has_edge(elem, c) for c in mwis_set)

            if all(feasible_iter):
                mwis_set += (elem,)
                mwis_weight += pi[elem]

                if mwis_weight > threshold + EPS and mwis_weight > best_score + EPS:
    ...
    # KEY FIX: always yield best for all_solutions=False
...
def greedy_MWIS(graph, pi, threshold, n_shuffle, all_solutions=False):
    ...
    It returns:
    - mwis_set, a MWIS as a sorted list of nodes
            ...
            if all(feasible_iter):
                ...
                if mwis_weight > threshold + EPS:
                    if mwis_weight > best_score + EPS:
                        ...
                        if all_solutions:
                            yield list(sorted(best_score_set)), best_score
...
def exact_MWIS(graph, pi, b_score=0):
    ...
    global best_score
    allValuesPI = list(pi.values())
    assert sum(allValuesPI) > 0
